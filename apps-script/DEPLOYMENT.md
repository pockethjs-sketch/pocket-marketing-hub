...
- 백엔드: `2026-09-03-supabase-backup-v47` (현재 운영)
- Web App 배포: @104
- 운영 검증: health `READY`, writes 활성; 무극 23건·UND 78건 일정과 일별 간트 배열 대조 완료
- 권한 격리: UND는 UND 프로젝트 1개만 노출, 무극 직접 조회 거부, NS 권한 관리 거부
- 원본 보존: 기존 21개 운영 탭 유지
- 신규 백업 경로: `[백업] Supabase업무` 숨김·보호 탭 기록 후 전체 Drive 복제
- 최신 실검증: 2026-09-03 09:07 KST, Supabase 업무 101건·숨김·보호·전체 복제 manifest 검증 성공

## 배포 메모

- `supabase_task_backup`은 일반 세션이 아니라 백업 러너 비밀값을 검증하고 Supabase 스냅샷 개수까지 일치할 때만 전용 탭을 갱신합니다.
- 스냅샷 ID는 101건 업무 내용의 SHA-256입니다. 같은 날이라도 마지막 전체 백업과 해시가 다르면 새 Drive 복제본을 만들고, 같으면 중복 생성을 건너뜁니다.
- 운영 원장 21개 탭은 이 동작으로 수정하지 않습니다.
