...
운영 데이터는 Supabase와 비공개 Google Sheets에 도메인별로 분리 저장합니다. 로그인·업무·업무 로그·회의록·KPI·권한·실행계획은 Supabase Auth/RLS/Postgres를 사용하고, 콘텐츠·성과 추적·파일·세부 로그는 전환이 끝날 때까지 Apps Script/Sheets를 유지합니다. 비밀키·접근코드·서명키는 공개 저장소에 넣지 않습니다.
...
- Supabase: Auth, 프로젝트 멤버십, 업무 101건, 업무 로그, 회의록, KPI, 고객 권한, 실행계획, mutation
- Google Sheets 원장: 콘텐츠·성과 추적·파일·세부 로그 및 업무·실행계획 롤백 대조본
- Schema version: `2026-08-26-v3`

현재 운영 원장과 화면에는 UND·무극·포켓컴퍼니 고객사·프로젝트를 유지합니다. 포켓컴퍼니 내부 프로젝트는 빈 원장으로 시작하고 Pocket 관리자에게만 기본 노출합니다. 2026-09-02 기존 업무를 전체 백업 후 초기화한 다음 기준 파일 `캠페인 스케줄 관리.html`의 무극 23건과 UND 78건을 `06_업무`에 다시 이관했습니다. 제목·세부내용·시작일·종료일·날짜별 간트 배열·상태·담당·완료링크·비고를 원본과 대조했고 두 프로젝트의 `source_task_id` 중복은 0건입니다. 최종 이관 보정 직전 백업 `PocketMarketingHub_20260902_195807`은 21개 시트 해시가 모두 일치하며, 동일 원본 재실행은 변경 없이 건너뜁니다.
...
- 포켓·NS의 상단 프로젝트 생성과 생성자 편집 권한 자동 연결
- 견적서 PDF·엑셀·CSV·TSV를 검토해 현재 프로젝트에 업무를 추가하거나 새 프로젝트를 원자적으로 생성
- 일정표·간트의 업무 수정 및 복구 가능한 보관 삭제
- Shift 범위 선택, 선택 업무 묶음 이동·일괄 삭제, 드롭 삽입선·자동 스크롤, 간트 행·날짜 축 강조
