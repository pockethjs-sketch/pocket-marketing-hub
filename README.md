...
운영 데이터는 Supabase와 비공개 Google Sheets에 도메인별로 분리 저장합니다. 업무·업무 로그는 Supabase Auth/RLS/Postgres를 사용하고, 실행계획·회의록·콘텐츠·성과·파일·권한은 전환이 끝날 때까지 Apps Script/Sheets를 유지합니다. 비밀키·접근코드·서명키는 공개 저장소에 넣지 않습니다.
...
- Supabase: Auth, 프로젝트 멤버십, 업무 101건, 업무 로그, 업무 mutation
- Google Sheets 원장: 실행계획·회의록·콘텐츠·성과·파일·권한 및 업무 롤백 대조본
