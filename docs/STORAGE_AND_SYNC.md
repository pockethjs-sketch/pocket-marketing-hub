...
- 견적 파일은 브라우저에서만 파싱하고 원본 파일 자체를 Supabase Storage나 Google Drive에 업로드하지 않는다. 저장 대상은 검토·선택한 견적 메타데이터와 업무 행이다.
- 새 프로젝트는 `create_project_from_quote`, 기존 프로젝트 추가는 `import_quote_tasks` RPC 한 번으로 저장한다. 두 경로 모두 프로젝트 권한 확인, mutation 중복 확인, `quote_data` 갱신, 업무 N건 생성, 활동 로그를 한 PostgreSQL 트랜잭션에서 처리한다.
- 일부 업무가 유효하지 않으면 앞에서 생성한 업무까지 전부 취소한다. 따라서 저장 실패 뒤 사용자가 같은 요청을 다시 실행해도 부분 행이나 중복 프로젝트가 남지 않는다.
- 파일 파싱 단계에는 저장 잠금을 띄우지 않고, 사용자가 최종 생성 버튼을 누른 뒤의 단일 네트워크 요청에만 전체 저장 잠금을 사용한다.
...
- 일정표 아래 표는 Supabase `project_issues`의 프로젝트별 행을 표시합니다. 업무의 `remarks`에 합치지 않으며 다른 프로젝트 행과도 섞지 않습니다.
- `이슈 행 추가`는 오늘 날짜와 `IN_PROGRESS` 상태의 빈 행을 먼저 서버에 생성합니다. 이후 날짜·텍스트·링크는 포커스가 빠질 때 변경 필드만 저장하고, 상태는 `예정 → 진행중 → 완료 → 보류` 순으로 눌러서 저장합니다.
- 삭제는 오조작 방지를 위해 같은 행의 삭제 버튼을 두 번 눌러야 하며 실제 삭제 대신 `archived_at`을 기록합니다. 활성 조회는 보관된 행을 제외합니다.
- 모든 쓰기는 `mutation_id` 중복 방지와 `row_version` 낙관적 잠금을 적용합니다. 완료링크는 빈 값 또는 `http://`·`https://`만 허용하고 서버가 돌려준 최종 행을 현재 업무 캐시에 병합하므로 저장 후 전체 목록을 다시 읽지 않습니다.
- 업무 페이지 최초 조회는 `read_task_workspace` 한 요청으로 업무·멤버·프로젝트·이슈를 함께 받습니다. 이슈 표를 추가했다는 이유로 Google Sheets나 Supabase 요청을 하나 더 보내지 않습니다.
