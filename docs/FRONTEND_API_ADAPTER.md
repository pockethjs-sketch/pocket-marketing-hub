...
포켓·NS 관리 계정에는 프로젝트 메뉴 하단에 `권한 관리`가 표시됩니다. 고객 계정의 허용 페이지는 bootstrap의 프로젝트별 `allowed_pages`로 전달되며, 프런트는 메뉴와 직접 URL 접근을 제한합니다. 프로젝트 하위 `업무 / 진행상황 / 데일리 회의록`도 각각 독립 코드(`tasks / progress / daily`)로 판정합니다. 서버도 같은 권한을 검사하므로 프런트 우회만으로 숨긴 페이지 데이터를 읽을 수 없습니다.
