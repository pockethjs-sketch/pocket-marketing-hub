...
`frontend/src/api/`의 API 계층을 `App.jsx`에 연결했습니다. 운영 빌드는 Apps Script를 통해 Google Sheets 원장을 읽고 쓰며, API 주소가 없거나 `demo` 모드를 명시한 로컬 환경에서만 비식별 샘플을 사용합니다.

어댑터가 지원하는 화면 단위는 다음과 같습니다.

| 프런트 메서드 | 서버 action | 용도 |
|---|---|---|
| `bootstrap` | `bootstrap` | 로그인 사용자가 볼 수 있는 고객사·프로젝트 |
...
const unsubscribe = source.subscribe((state) => {
  // state.mode: initializing | live | offline-demo
...
`auto` 모드는 API URL 자체가 없을 때만 비식별 샘플을 사용합니다. URL이 설정된 뒤 발생한 인증·네트워크·서버 오류는 데모 데이터로 감추지 않고 오류로 표시합니다. 데모 모드의 쓰기는 성공한 것처럼 보이지 않게 `OfflineMutationError`로 차단합니다.
...
1. 앱 부팅에서 `bootstrap()`을 호출하고 고객사·프로젝트 목록을 교체합니다.
2. 선택 프로젝트가 바뀌면 `overview()`를 호출합니다.
3. 각 탭 진입 시 해당 화면 action을 지연 조회합니다.
4. 화면에 `loading`, `error`, `offline-demo`, 마지막 성공 시각을 표시합니다.
