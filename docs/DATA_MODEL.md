...
고객 업무 응답에는 일정, 단계, 분야, 분류, 공개 상태, `plan_week`, 공개 진행 메모와 `contract_linked` 여부를 포함할 수 있습니다. `plan_note`, 내부 설명, 내부 담당자 ID, 검토 조직, 차단 사유와 원천 코드는 제외합니다. 계약 연계 여부가 `true`여도 계약·검수 사유 원문은 노출하지 않습니다.
