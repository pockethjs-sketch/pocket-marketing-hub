...
일정표 필터는 별도 저장값을 만들지 않고 `06_업무`의 원천 필드로만 계산합니다. 상태는 `status_code`, 카테고리는 `category_code`의 화면 라벨, 일정 구간은 `planned_start_date`·`due_date`와 오늘 날짜를 사용합니다. `오늘 진행`은 오늘이 시작일~종료일에 포함된 미완료 업무, `예정`은 시작일이 오늘 이후인 미완료 업무, `지연`은 종료일이 지났지만 완료되지 않은 업무, `일정 미등록`은 시작일 또는 종료일이 없는 업무입니다. 진행률은 필터 판정에 사용하지 않습니다.
