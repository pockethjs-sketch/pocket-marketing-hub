...
업무 `일정표` 화면은 `업무 / 세부내용 / 시작일 / 종료일 / 기간(일) / 진행률 / 상태 / 담당 / 완료링크 / 비고` 10개 열과 일 단위 캘린더를 같은 행으로 연결합니다. 담당은 `responsible_org_code`를 현재 프로젝트 고객사명과 조합해 표시합니다. 각 업무의 시작일~종료일은 오른쪽 날짜 셀에 색으로 채워지고, 월·일·요일과 오늘 기준선을 함께 표시합니다. 기간은 시작일·종료일을 포함해 화면에서 계산하고 별도 중복 필드로 저장하지 않습니다. Apps Script는 Sheets Date 객체를 KST `yyyy-MM-dd`로 정규화해 브라우저 시간대에 따른 하루 밀림을 방지합니다.
...
일정표 날짜축의 끝은 현재 표시 업무 중 가장 늦은 `due_date`로 제한합니다. 프로젝트 `end_date`가 더 늦더라도 실제 업무가 없는 빈 날짜 구간은 만들지 않습니다. 날짜가 등록된 업무가 하나도 없는 경우에만 프로젝트 시작일·종료일을 예비 축으로 사용합니다.
