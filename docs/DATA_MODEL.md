...
업무 `표·일정` 화면은 `업무 / 세부내용 / 시작일 / 종료일 / 기간(일) / 진행률 / 상태 / 완료링크 / 비고` 9개 열과 일 단위 간트를 함께 표시합니다. 기간은 시작일·종료일을 포함해 화면에서 계산하고 별도 중복 필드로 저장하지 않습니다. Apps Script는 Sheets Date 객체를 KST `yyyy-MM-dd`로 정규화해 브라우저 시간대에 따른 하루 밀림을 방지합니다.
