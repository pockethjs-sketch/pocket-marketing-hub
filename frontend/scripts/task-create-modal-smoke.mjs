...
try {
  ...
  const dateStyle=await evaluate('(()=>{const fields=[...document.querySelectorAll(".project-issue-date-field")],inputs=fields.map(el=>el.querySelector("input")),styles=inputs.map(el=>getComputedStyle(el));return {labels:fields.map(el=>el.querySelector("span").textContent),labelSizes:fields.map(el=>getComputedStyle(el).fontSize),dateSizes:styles.map(s=>s.fontSize),dateWeights:styles.map(s=>s.fontWeight),normalBackground:styles[0].backgroundColor,deadlineBackground:styles[1].backgroundColor,deadlineColor:styles[1].color,deadlineHeight:inputs[1].getBoundingClientRect().height,rowHeight:fields[0].closest("tr").getBoundingClientRect().height};})()');
  assert.deepEqual(dateStyle.labels,["등록일","컨펌 마감일"]);assert.equal(new Set(dateStyle.labelSizes).size,1);assert.equal(new Set(dateStyle.dateSizes).size,1);assert.equal(new Set(dateStyle.dateWeights).size,1);assert.notEqual(dateStyle.normalBackground,dateStyle.deadlineBackground);assert.notEqual(dateStyle.deadlineBackground,"rgba(0, 0, 0, 0)");assert.ok(dateStyle.deadlineHeight>=29&&dateStyle.rowHeight>=68,JSON.stringify(dateStyle));assert.ok(await evaluate('[...document.querySelectorAll(".project-issue-date-field input")].every(el=>el.clientWidth>=125)'));
  if(process.env.QA_ISSUE_SCREENSHOT){await evaluate('document.querySelector(".project-issue-panel").scrollIntoView({block:"start"})');const shot=await send("Page.captureScreenshot",{format:"png"});await writeFile(process.env.QA_ISSUE_SCREENSHOT,Buffer.from(shot.data,"base64"));}
  ...
  console.log("PASS: date presets, validation, owner/status selection, duplicate-submit prevention, failed-draft retention, completed 100%, equal issue-date typography and highlighted deadline, issue button and mobile layout.",JSON.stringify({desktop,issueStyle,dateStyle}));
